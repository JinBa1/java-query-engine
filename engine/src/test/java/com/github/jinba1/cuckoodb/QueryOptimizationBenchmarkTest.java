 ...
 */
public class QueryOptimizationBenchmarkTest extends CuckooDBTest {
    ...
    private long runQueryWithTupleCount(String queryName, String queryContent, boolean optimize) throws IOException {
        ...
        // Parse and execute. Nested-loop join on both paths: this benchmark isolates the
        // optimizer's effect on scan counts; join-algorithm gains are measured by the JMH suite
        Operator rootOp = QueryPlanner.parseStatement(queryFilePath, new QueryConfig(optimize, false));
        ...
        CuckooDB.execute(rootOp, outputFilePath);

        // Collect scan tuple count (measures actual data read from disk)
        return collectScanTupleCount(rootOp);
    }
    ...
    private void resetAllTupleCounters(Operator op) {
        ...
        if (op instanceof com.github.jinba1.cuckoodb.operator.JoinOperator) {
            resetAllTupleCounters(((com.github.jinba1.cuckoodb.operator.JoinOperator) op).getOuterChild());
        }
    }
    ...
    private long collectScanTupleCount(Operator op) {
        ...
        if (op instanceof com.github.jinba1.cuckoodb.operator.ScanOperator) {
        ...
        if (op instanceof com.github.jinba1.cuckoodb.operator.JoinOperator) {
            count += collectScanTupleCount(((com.github.jinba1.cuckoodb.operator.JoinOperator) op).getOuterChild());
        }
